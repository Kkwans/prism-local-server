...
配置管理模块
负责配置文件的读取、保存和验证
配置文件存储在用户目录: C:\\Users\\{username}\\.prism-server\\
作者: Kkwans
创建时间: 2026-03-16
修改时间: 2026-03-16 (改为用户目录存储)
"""
...
DEFAULT_DIRECTORY = ""  # 默认部署目录(空表示当前工作目录)
...
class ConfigManager:
    ...
    - 验证配置有效性
    - 提供默认配置
    - 配置文件存储在用户目录,确保EXE独立运行
    """
    
    def __init__(self):
        """
        初始化配置管理器
        配置目录: C:\\Users\\{username}\\.prism-server\\
        """
        # 获取用户主目录
        user_home = Path.home()
        
        # 配置目录路径
        self.config_dir = user_home / CONFIG_DIR_NAME
